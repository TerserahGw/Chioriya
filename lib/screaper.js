...
async function aiTranslate(query) {
    return new Promise(async (resolve, reject) => {
        try {
            const response = await axios.post('https://www.basedgpt.chat/api/chat', JSON.stringify({
                messages: [{ 
                    role: 'user', 
                    content: `Terjemahkan teks berikut ke dalam Bahasa Indonesia (cukup jawab dengan teks yang sudah diterjemahkan tanpa hal lain seperti simbol dan kata tambahan Contohnya jika aku kirim What, kamu nanti mengirim Apa): [${query}]` 
                }],
            }), {
                headers: {
                    'Content-Type': 'application/json',
...
module.exports = { chatGpt, aiTranslate, tiktokDl, facebookDl, instaDl, instaStory, ytMp4, ytMp3, Ytdl }