...
const gameCasinoSolo = async (conn, m, prefix, db) => {
   try {
      ...
      if (db[m.sender].uang >= count * 1) {
         ...
         if (Aku > Kamu) {
            m.reply(`💰 Casino 💰\n*Kamu:* ${Kamu} Point\n*Computer:* ${Aku} Point\n\n*You LOSE*\nKamu kehilangan ${count} Primogems`.trim())
         } else if (Aku < Kamu) {
            db[m.sender].uang += count * 2
            m.reply(`💰 Casino 💰\n*Kamu:* ${Kamu} Point\n*Computer:* ${Aku} Point\n\n*You Win*\nKamu mendapatkan ${count * 2} Primogems`.trim())
         } else {
            db[m.sender].uang += count * 1
            m.reply(`💰 Casino 💰\n*Kamu:* ${Kamu} Point\n*Computer:* ${Aku} Point\n\n*SERI*\nKamu mendapatkan ${count * 1} Primogems`.trim())
         }
      } else { 
          m.reply(`Primogems kamu tidak mencukupi untuk Casino silahkan *kumpulkan* terlebih dahulu!`)
     }
   } catch (e) {
   	console.log(e)
      m.reply('Error!!')
...
const gameMerampok = async (m, db) => {
   ...
   if (new Date - db[m.sender].lastrampok > 3600000) {
      if (10000 > db[who].uang) return m.reply('Dia Miskin Bang🗿')
      db[who].uang -= dapat
      db[m.sender].uang += dapat
      db[m.sender].lastrampok = new Date * 1
      m.reply(`Berhasil Merampok Primogems Target Sebesar ${dapat}`)
   } else {
...
const gameTangkapOr = async (conn, m, db) => {
    ...
    let _timers = (3600000 - __timers)
    let timers = clockString(_timers)
	const randomUang = Math.floor(Math.random() * 1001)
	...
	if (new Date - user.lasttkpor > 3600000) {
		...
		if (teksnya.no === 0) {
			...
			await conn.sendMessage(m.chat, { text: teksnya.teks, edit: key })
			await conn.sendMessage(m.chat, { text: `Kamu Di Bunuh Oleh Pemain\nPrimogems Kamu Di Rampas Sebesar *${randomUang}*` }, { quoted: m })
			db[m.sender].uang -= randomUang
		} else {
			await conn.sendMessage(m.chat, { text: teksnya.teks, edit: key })
			await conn.sendMessage(m.chat, { text: `Berhasil Mendapatkan Primogems Sebesar : *${randomUang}*` }, { quoted: m })
			db[m.sender].uang += randomUang
	...
	if (isNaN(args[0])) {
	  ...
	  if (db[m.sender].limit >= count * 1) {
		  try {
            ...
            conn.sendMessage(m.chat, { text: `Berhasil mentransfer Stamina sebesar ${count}, kepada @${who.split('@')[0]}`, mentions: [who] }, { quoted : m }) 
          } catch (e) {
            db[m.sender].limit += count * 1
            m.reply('Gagal Menstransfer')
          }
      } else {
    	  m.reply(`Stamina tidak mencukupi!!\nStamina mu tersisa : *${db[m.sender].limit}*`)
      }
   }
}
	...
	if (isNaN(args[0])) {
	  ...
	  if (db[m.sender].uang >= count * 1) {
		  try {
            db[m.sender].uang -= count * 1
            db[who].uang += count * 1
            conn.sendMessage(m.chat, { text: `Berhasil mentransfer primogems sebesar ${count}, kepada @${who.split('@')[0]}`, mentions: [who] }, { quoted : m }) 
          } catch (e) {
            db[m.sender].uang += count * 1
            m.reply('Gagal Menstransfer')
          }
      } else {
    	  m.reply(`Primogems tidak mencukupi!!\nPrimogems mu tersisa : *${db[m.sender].uang}*`)
      }
   }
}
